...
*/
void test_run2(hunt.io.SimpleQueue.Queue!long q, uint threads) {
    const count = MagedMichaelOpCount / threads;
    Thread[] ts;
    foreach (i; 0 .. threads) {
        auto t = new Thread({
...
class LinkedBlockingQueueBench {
    ...
    void bench() {
        ...
        void f2() { test_run2(new hunt.io.SimpleQueue.BlockingQueue!long(),                writers); }
        void f3() { test_run2(new hunt.io.SimpleQueue.NonBlockingQueue!long(),             writers); }
