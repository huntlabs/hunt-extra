...
module hunt.stream.BufferedInputStream;
...
class BufferedInputStream : FilterInputStream {
    ...
    private enum int DEFAULT_BUFFER_SIZE = 8192;
     ...
     */
    private enum int MAX_BUFFER_SIZE = int.max - 8;
     ...
     * <code>inputStream</code>, for later use. An internal
    ...
    private void fill() {
        ...
        count = pos;
        int n = getInIfOpen().read(buffer, pos, cast(int)buffer.length - pos);
        if (n > 0)
            count = n + pos;
    }
    ...
    private int read1(byte[] b, int off, int len) {
        ...
        b[off.. off+cnt] = getBufIfOpen()[pos .. pos+cnt];
    ...
    override int read(byte[] b, int off, int len) {
        ...
        for (;;) {
            ...
            InputStream input = inputStream;
            if (input !is null && input.available() <= 0)
                return n;
        }
    }
     ...
     *                      invoking its {@link #close()} method,
     *                      {@code inputStream.skip(n)} throws an IOException,
     *                      or an I/O error occurs.
     */
     ...
     * {@link java.io.FilterInputStream#inputStream inputStream}.available().
     *
    ...
    override int available() {
        ...
        return n > (int.max - avail) ? int.max : n + avail;
    }
     ...
     * <code>true</code>.
     *
     * @return  a <code>bool</code> indicating if this stream type supports
