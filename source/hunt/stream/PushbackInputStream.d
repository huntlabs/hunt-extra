...
module hunt.stream.PushbackInputStream;
...
class PushbackInputStream : FilterInputStream {
     ...
     * byte is returned as an <code>int</code> inputStream the range
    ...
    public int read(byte[] b, int off, int len)  {
        ...
        int avail = cast(int)(buf.length - pos);
        if (avail > 0) {
            if (len < avail) {
                avail = len;
            }
            // System.arraycopy(buf, pos, b, off, avail);
            b[off..off+avail] = buf[pos..pos + avail];
            pos += avail;
            off += avail;
            len -= avail;
        }
     ...
     * @exception IOException If there is not enough room inputStream the pushback
     ...
     * first skips over the bytes inputStream the pushback buffer, if any.  It then
